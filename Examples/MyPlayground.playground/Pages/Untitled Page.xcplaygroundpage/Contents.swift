...
class BuildingSktch: Sketch, SketchSample {
  ...
  override func setup() {
    ...
    for room in allRooms {
      ...
      if let edge = notAdjescentEdges.randomElement() {
        ...
        let doorPosition = lerp(start: wall.a.interpolte(to: wall.b, by: 10),
                                stop: wall.b.interpolte(to: wall.a, by: 10),
                                amount: random())
  ...
  override func draw() {
    ...
    for door in appartment.doors {
      ...
      strokeWeight(2)
      arc(Arc(center: door.position, radius: 7, start: start, stop: stop))
...
let sketch = BuildingSktch(size: Size(width: 600, height: 600))
